---
metaLinks:
  alternates:
    - >-
      https://app.gitbook.com/s/hbuQuZovtBBsMP54qBxh/inner-pages/search-results/blog-card/button
---

# Button

<figure><img src="../../../.gitbook/assets/Screenshot_105.png" alt=""><figcaption></figcaption></figure>

|                      |                                                                                                                                                                             |
| -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Text                 | Add the text to the button                                                                                                                                                  |
| Style                | <p>Select the button style. (Primary, Secondary, Link ) </p><p>Note: According to the selected button style, button background and hover background color gets changed.</p> |
| Text case            | Use the Text case setting to adjust the text style. (Uppercase, Capitalize, Default)                                                                                        |
| Open link in new tab | Enable to open the link in a new browser tab.                                                                                                                               |
